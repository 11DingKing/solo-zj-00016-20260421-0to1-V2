    ...
    <div class="container mx-auto px-4 py-8 max-w-4xl">
      ...
      <section class="bg-white rounded-2xl shadow-lg p-6 mb-8">
        ...
        <form @submit.prevent="createShortLink" class="space-y-4">
          ...
          <div>
            <label class="block text-sm font-medium text-gray-600 mb-1">过期时间</label>
            <select
          ...
          <div v-if="expiryOption === 'custom'">
            <label class="block text-sm font-medium text-gray-600 mb-1">选择过期日期和时间</label>
            <input
              type="datetime-local"
        ...
        >
          ...
          <div class="flex flex-col md:flex-row items-start md:items-center gap-4">
            <div class="flex-1">
              ...
              <p v-if="createdLink.expires_at" class="text-sm text-gray-500 mt-1">
                过期时间：{{ formatDate(createdLink.expires_at) }}
              </p>
            </div>
      ...
      <section class="bg-white rounded-2xl shadow-lg p-6">
        ...
        <div v-if="isLoadingLinks" class="text-center py-12 text-gray-500">
          加载中...
        </div>

        <div v-else-if="userLinks.length === 0" class="text-center py-12 text-gray-500">
            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div class="flex-1 min-w-0">
                <div class="flex items-center gap-2 mb-1">
                  ...
                  <span
                    v-else-if="link.expires_at"
                    class="px-2 py-0.5 text-xs font-medium bg-green-100 text-green-600 rounded-full"
                  >
                    有效
                  </span>
                </div>
                ...
                <div class="flex items-center flex-wrap gap-4 mt-2 text-xs text-gray-400">
                  ...
                  <span v-if="link.expires_at">
                    {{ link.is_expired ? '已过期' : '过期: ' + formatDate(link.expires_at) }}
                  </span>
                </div>
              </div>
              <div class="flex items-center gap-2">
        ...
        <div
          v-if="totalPages > 1"
          class="flex items-center justify-center gap-2 mt-6 pt-4 border-t border-gray-100"
        >
          <button
...
const longUrl = ref('');
const expiryOption = ref('permanent');
...
const getExpiryParams = () => {
  switch (expiryOption.value) {
    case '7days':
      return { expires_in_hours: 168, expires_at: null };
    case '30days':
      return { expires_in_hours: 720, expires_at: null };
    case 'custom':
      if (customExpiry.value) {
        return { expires_in_hours: null, expires_at: new Date(customExpiry.value) };
      }
      return { expires_in_hours: null, expires_at: null };
    default:
      return { expires_in_hours: null, expires_at: null };
  }
};
